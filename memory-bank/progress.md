...
| Ready-check decline false positive (issue #169) | ✅ Done | `RoomViewModel` now only opens the local timeout modal after an actual `Timeout`; room clear while still `Pending` returns quietly to matchmaking when someone else declines |

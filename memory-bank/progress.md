...
| Ready-check decline false positive (issue #169) | ✅ Done | `RoomViewModel` now resolves cleared ready-check rooms from the subsequent queue state: `Pending` + requeued returns quietly to matchmaking, while `Pending` + not requeued still shows the local timeout modal |

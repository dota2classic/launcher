...
| #169 | Ready-check decline false positive — backend never sends a distinct `Timeout` state to the launcher, so `RoomViewModel` now treats room-clear while the player is still `Pending` as a short-lived unresolved state and classifies it from the follow-up `PLAYER_QUEUE_STATE`: requeued (`InQueue=true`) means someone else declined and no modal is shown, not requeued means show the local "you did not accept" timeout modal |

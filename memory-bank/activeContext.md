...
| #169 | Ready-check decline false positive — `RoomViewModel` now only shows the local "you did not accept" timeout modal when the player's last state was `Timeout`; if the room disappears while the player is still `Pending` because someone else declined, the launcher just returns to matchmaking without the stale modal |
